...
const Table = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:4000/api/getproductFilter");
        if (!response.ok) {
            ...
            {currentProducts.map((product) => (
              <tr key={product.id} className='bg-white border-b'>
                ...
                <td className='inline-flex px-6 gap-2 py-4'>
                  <Link to="/admin/products/viewproducts" className='text-lightsuccess pointer'>
                    <GrFormView size={20} />
                  </Link>
                  <Link to="/admin/products/editproducts">
                    <FaRegEdit className='text-green-400 pointer' size={20} />
                  </Link>
                  <Link to="">
                    <MdOutlineDeleteOutline className='text-red-400 pointer' size={20} />
                  </Link>
                </td>
              </tr>
            ))}
